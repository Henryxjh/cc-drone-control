...
基于 CC:Tweaked、GPS、Rednet 与有线 `electric_motor` 的四旋翼无人机控制脚本。
控制器直接输出电机速度，并通过四个辅助控制器获取螺旋桨坐标，实现悬停、手动移动、
自动导航和安全停机。
...
方案时，可根据实际安装方式调整电机外设名和配置。

## 分支

- `electric_motor`：使用有线 `electric_motor` 控制速度，并用 Rednet 获取姿态辅助坐标的主要版本
...
2. 在四个姿态辅助控制器中分别放入 `side.lua`，并修改顶部的 `controller` 和 `position`。
3. 将 `power.lua` 放入电源控制器，并修改顶部的主控制器 ID。
4. 修改 `controller-config.lua` 中的 modem、外设方向、计算机 ID、动力范围和悬停参数。
